...
public class PlaybackControlView extends FrameLayout {
  ...
  private VideoTimeCallback videoCallBackListener;
    /**Custom method to set the listener listening to timeEvents*/

  public void setVideoCallBackListener(VideoTimeCallback videoCallBackListener) {
   ...
   */
  // Custom implementation to not remove the callback when Playback controller is getting hidden
  // Taking care of removing callback when when the activity containing player stops.
  public void hide(boolean removeCallback) {
    if (isVisible()) {
      setVisibility(GONE);
      if (visibilityListener != null) {
        visibilityListener.onVisibilityChange(getVisibility());
  ...
  /**Modification in update progress to get callbacks at all times
   * not just when timebar is visible*/
  private void updateProgress() {
    if (!isAttachedToWindow) {
      ...
      //Modified not updating the view just returning the callback
    if(!isVisible()) {
        if (player != null) {
            ...
            if (videoCallBackListener != null) {
                videoCallBackListener.timeReached(position);
            }
